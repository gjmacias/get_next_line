...
/*   Updated: 2022/03/27 11:07:39 by gmacias-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*line;
	static char	*save_fds[4096];

	if (fd < 0 || fd > 4095 || BUFFER_SIZE <= 0)
		return (NULL);
	save_fds[fd] = read_until_enter(fd, save_fds[fd]);
	if (save_fds[fd] == NULL)
		return (NULL);
	line = make_line_from(save_fds[fd]);
	save_fds[fd] = the_rest(save_fds[fd]);
	return (line);
}
